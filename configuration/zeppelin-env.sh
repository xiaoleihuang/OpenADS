...
# export JAVA_HOME=
export MASTER=spark://$HOSTNAME:7077          		# Spark master url. eg. spark://master_addr:7077. Leave empty if you want to use local mode.
...
export SPARK_SUBMIT_OPTIONS --jars /OpenADS/target/OpenADS-0.1-SNAPSHOT-jar-with-dependencies.jar    # (optional) extra options to pass to spark submit. eg) "--driver-memory 512M --executor-memory 1G".
...
export HADOOP_CONF_DIR=/usr/local/hadoop/etc/hadoop  	# yarn-site.xml is located in configuration directory in HADOOP_CONF_DIR.
